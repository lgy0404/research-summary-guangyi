...
const shadow = { type: 'outer', color: '000000', opacity: 0.16, blur: 2, angle: 45, distance: 1 };
...
function addCard(slide, x, y, w, h, fill = C.white, line = C.line, radius = 0.08) {
  slide.addShape(slide._pptx.ShapeType.roundRect, {
    ...
    fill: { color: fill }, line: { color: line, width: 1 }, shadow,
  });
}

function addTop(slide, title, font, kicker = '') {
  slide.addShape(slide._pptx.ShapeType.rect, { x: 0, y: 0, w: W, h: 0.86, fill: { color: C.blue }, line: { color: C.blue } });
  if (kicker) addText(slide, kicker.toUpperCase(), 0.34, 0.13, 1.55, 0.18, 8.5, 'BFC7FF', true, { fontFace: font, charSpacing: 1.2 });
  addText(slide, title, kicker ? 1.72 : 0.42, 0.14, kicker ? 11.15 : 12.1, 0.52, 24, C.white, false, { fontFace: font });
}
...
function addPill(slide, text, x, y, w, fill, color, font, size = 10) {
  slide.addShape(slide._pptx.ShapeType.roundRect, { x, y, w, h: 0.32, fill: { color: fill }, line: { color: fill } });
...
function addMaster(pptx, lang, font) {
  ...
  pptx.defineSlideMaster({
    ...
    objects: [
      { rect: { x: 0, y: 7.2, w: 4.42, h: 0.3, fill: { color: C.navy }, line: { color: C.navy } } },
      { rect: { x: 4.42, y: 7.2, w: 4.45, h: 0.3, fill: { color: '252B86' }, line: { color: '252B86' } } },
      { rect: { x: 8.87, y: 7.2, w: 4.463, h: 0.3, fill: { color: C.blue }, line: { color: C.blue } } },
      { text: { text: author, options: { x: 0.25, y: 7.245, w: 4.0, h: 0.13, fontFace: font, fontSize: 8, color: C.white, align: 'center', margin: 0 } } },
      { text: { text: center, options: { x: 4.55, y: 7.245, w: 4.18, h: 0.13, fontFace: font, fontSize: 7.5, color: C.white, align: 'center', margin: 0 } } },
      { text: { text: `/${TOTAL}`, options: { x: 12.72, y: 7.235, w: 0.32, h: 0.15, fontFace: font, fontSize: 8, color: C.white, margin: 0 } } },
    ],
    slideNumber: { x: 12.34, y: 7.235, w: 0.38, h: 0.15, fontFace: font, fontSize: 8, color: C.white, align: 'right', margin: 0 },
  });
}
...
function addCover(pptx, d, font) {
  ...
  s.addShape(pptx.ShapeType.ellipse, { x: 9.4, y: 0.8, w: 4.6, h: 4.6, fill: { color: 'E2E6FF', transparency: 12 }, line: { color: 'E2E6FF', transparency: 100 } });
  s.addShape(pptx.ShapeType.roundRect, { x: 0.62, y: 1.05, w: 8.62, h: 2.08, fill: { color: C.blue }, line: { color: C.blue }, shadow });
  addText(s, d.coverTitle, 0.94, 1.35, 7.96, 1.15, d.lang === 'zh' ? 28 : 27, C.white, true, { fontFace: font });
  addText(s, d.coverSubtitle, 0.95, 2.52, 7.9, 0.34, 12.5, 'D9DEFF', false, { fontFace: font });
  s.addShape(pptx.ShapeType.roundRect, { x: 9.75, y: 1.26, w: 2.55, h: 3.08, fill: { color: C.white }, line: { color: C.white }, shadow });
  s.addImage({ path: PROFILE, x: 10.18, y: 1.58, w: 1.7, h: 1.72 });
  addText(s, d.name, 9.92, 3.48, 2.2, 0.38, 18, C.navy, true, { fontFace: font, align: 'center' });
  addText(s, d.role, 9.8, 3.84, 2.45, 0.32, 9.5, C.muted, false, { fontFace: font, align: 'center' });
  d.coverPills.forEach((p, i) => addPill(s, p, 0.78 + i * 2.25, 3.62, 2.0, i === 0 ? 'E5E9FF' : i === 1 ? 'E6F6F7' : 'F9EBDD', i === 0 ? C.blue : i === 1 ? C.cyan : C.orange, font, 9.5));
  addText(s, d.coverLine, 0.82, 4.36, 7.9, 0.6, 18, C.ink, true, { fontFace: font });
  addText(s, d.coverFoot, 0.82, 5.05, 7.8, 0.42, 11, C.muted, false, { fontFace: font });
...
function addPaperSection(pptx, d, font, p) {
  const s = pptx.addSlide(`MASTER_${d.lang}`);
  s.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: W, h: 7.2, fill: { color: C.navy }, line: { color: C.navy } });
  s.addShape(pptx.ShapeType.ellipse, { x: 9.65, y: 0.45, w: 4.9, h: 4.9, fill: { color: p.accent, transparency: 72 }, line: { color: p.accent, transparency: 100 } });
  addText(s, p.index, 0.72, 0.72, 2.2, 0.55, 25, p.accent, true, { fontFace: font, charSpacing: 1.4 });
  addText(s, p.title, 0.72, 1.55, 11.1, 1.28, d.lang === 'zh' ? 27 : 25, C.white, true, { fontFace: font });
  addText(s, p.subtitle, 0.75, 2.9, 10.65, 0.72, 17, 'D8DDF6', false, { fontFace: font });
  addCard(s, 0.76, 4.05, 8.55, 1.25, '222965', '3A438D');
  addText(s, p.problemLabel, 1.02, 4.28, 1.25, 0.28, 10, p.accent, true, { fontFace: font });
  addText(s, p.problem, 2.1, 4.22, 6.83, 0.72, 15.2, C.white, true, { fontFace: font });
  p.tags.forEach((t, i) => addPill(s, t, 0.76 + i * 2.1, 5.72, 1.86, i === 0 ? p.accent : '343D86', C.white, font, 9));
  addText(s, p.citation, 9.75, 5.86, 2.55, 0.28, 8.7, 'C1C7E7', false, { fontFace: font, align: 'right' });
...
async function build(lang) {
  ...
  await pptx.writeFile({ fileName: `${OUT}/GuangyiLiu-ZJU-codex-${suffix}.pptx` });
}
